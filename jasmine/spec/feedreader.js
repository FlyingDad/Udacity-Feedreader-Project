...
$(function () {
	...
	describe('New Feed Selection', function () {
		...
		*/
		...
		// store current feeds, then reload page
		beforeEach(function(done){
			// load first feed, and save links as nodelist, allow one second
			setTimeout(function(){			
				loadFeed(0, function(){
					preFeedContent = document.querySelectorAll('.feed a');
					done();
				});			
			}, 1000);
		})
		...
		it('new feed changes page content', function(done){
			...
			// load the second feed and save links as loadlist
			loadFeed(1, function(){
				postFeedContent = document.querySelector('.feed a');
				// comapre nodelist, they should be different
